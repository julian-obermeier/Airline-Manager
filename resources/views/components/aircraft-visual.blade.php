...
<div {{ $attributes->merge(['class' => 'aircraft-visual aircraft-visual-'.$group]) }}>
    <svg class="aircraft-side-profile" viewBox="0 0 340 125" role="img" aria-label="{{ $label ? 'Flugzeug '.$label : 'Flugzeug' }}">
        <defs>
            <linearGradient id="bodyGradient-{{ $group }}" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stop-color="#ffffff"/>
                <stop offset="72%" stop-color="#e7eef7"/>
                <stop offset="100%" stop-color="#cbd8e7"/>
            </linearGradient>
            <linearGradient id="wingGradient-{{ $group }}" x1="0" y1="0" x2="1" y2="1">
                <stop offset="0%" stop-color="#dbe7f4"/>
                <stop offset="100%" stop-color="#9fb4ca"/>
            </linearGradient>
            <filter id="shadow-{{ $group }}" x="-20%" y="-40%" width="140%" height="180%">
                <feDropShadow dx="0" dy="5" stdDeviation="4" flood-color="#27415f" flood-opacity=".18"/>
            </filter>
        </defs>

        <g filter="url(#shadow-{{ $group }})">
            ...
            @if($group === 'jumbo')
                <path d="M56 61 C82 44 103 41 140 43 L270 45 C296 46 313 54 322 63 C313 72 295 77 270 78 L98 79 C76 78 59 72 48 66 Z"
                      fill="url(#bodyGradient-{{ $group }})" stroke="#8fa6bd" stroke-width="1.2"/>
            ...
            <path d="M{{ $bodyEnd - 55 }} 48 L{{ $bodyEnd - 27 }} 18 L{{ $bodyEnd - 12 }} 18 L{{ $bodyEnd - 20 }} 50 Z"
                  fill="url(#wingGradient-{{ $group }})" stroke="#91a8c0" stroke-width="1"/>
